...
button_shutdown (){
    ...
    if [ $ret -eq 0 ]; then
        echo `date` "[WARNING] Pressed Shutdown Button"
...
button
while [ $? -eq 0 ]; do
    echo `date` "[Init] Waiting for GPIO Configuring ボタン状態を確認中"
    raspi-gpio set ${BUTTON_IO} ip pu
    sleep 1
    button
done

echo `date` "[Init] Connecting to LCD 起動中"
...
echo `date` "[Init] Started Main Loop メイン処理を開始"
wall \($0\) "Started RasPi Monitor"
while true; do
    # 時計
    echo `date` "[Loop] Clock 時計表示"
    hour=$((`/usr/bin/date "+%_I"`))
    hour_pt=$(( ( ( $hour + 6 ) % 12 ) * 100 / 12 ))
    /home/pi/RaspberryPi/gpio/raspi_lcd -i -n -b $hour_pt >/dev/null
    /home/pi/RaspberryPi/gpio/raspi_lcd -y2 `/usr/bin/date "+%R:%S"` >/dev/null
    sleep 2
    button_shutdown

    # UPTIME CPU負荷
    echo `date` "[Loop] CPU Load 負荷表示"
    cpu_f=`uptime|cut -d',' -f3|tr -d ' '|cut -d':' -f2`
    cpu100=`echo "${cpu_f} * 100 / ${CPU_CORES}"|bc`
    cpu_i=`printf "%.0f" $cpu100`
    echo `date` "[Value] CPU =" ${cpu_i}
    if [ $cpu_i -ge 80 ]; then
        echo `date` "[Caution] CPU負荷が高くなっています。現在、"${cpu_i}"％です。"
    ...
    if [ $mem_i -ge 90 ]; then
        if [ $mem_i -gt ${MEMS[1]} ]; then
            echo `date` "[WARNING] メモリ使用量の異常事態です。現在、"${mem_i}"％です。"
            wall \($0\) "[WARNING] Used Memory =" ${mem_i}
        else
            echo `date` "[Caution] メモリ使用量が高くなっています。現在、"${mem_i}"％です"
    ...
    # DF SDカード容量
    echo `date` "[Loop] Used SD Card SDカード容量表示"
    sdcap=`df | grep "/dev/root"|awk '{print $5}'|cut -d"%" -f1`
    sdcap_i=$(($sdcap))
    echo `date` "[Value] SSD =" ${sdcap_i}
    if [ $sdcap_i -lt 0 ]; then
        sdcap_i=0
    fi
    if [ $sdcap_i -ge 60 ]; then
        if [ $sdcap_i -ge 90 ]; then
            echo `date` "[WARNING] SDカード使用率の異常事態です。現在、"${sdcap_i}"です"
            wall \($0\) "[WARNING] Used Memory =" ${mem_i}
        else
            echo `date` "[Caution] SDカード使用率が"${sdcap_i}"％です"
            wall \($0\) "Used Memory =" ${mem_i}
        fi
    fi
